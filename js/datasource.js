...
class SimulatorSource {
  ...
  async start() {
    ...
    const loop = () => {
      ...
      this._timer = setTimeout(() => { this._step(); loop(); }, wait);
    };
    loop();
  }
  ...
  _step() {
    ...
    const q = (big ? (10 + Math.floor(Math.random() * 90)) : (1 + Math.floor(Math.random() * 9))) * 100;
    this.price = Math.max(t, this.price + steps * t);
    this.price = roundToTick(this.price);
    const side = steps > 0 ? 'B' : steps < 0 ? 'S' : (Math.random() < 0.5 ? 'B' : 'S');
  ...
  _emitBoard() {
    ...
    for (let i = 1; i <= 10; i++) {
      const askQ = Math.floor((300 + Math.random() * 4000) * (1 - cumBias * 0.12) * (1 + i * 0.08));
      const bidQ = Math.floor((300 + Math.random() * 4000) * (1 + cumBias * 0.12) * (1 + i * 0.08));
      asks.push([this.price + i * t, Math.max(100, Math.round(askQ / 100) * 100)]);
      bids.push([this.price - i * t, Math.max(100, Math.round(bidQ / 100) * 100)]);
    }
    this._emit({
      t: Date.now(), type: 'board', asks, bids,
...
class KabuSource {
  ...
  async start() {
    ...
    if (!r || !r.ok) throw new Error('kabu APIトークン取得失敗。kabuステーション起動とAPI設定を確認してください。');
    ...
    this._ws.onmessage = (m) => {
      try { this._handle(JSON.parse(m.data)); } catch (e) { /* skip */ }
    };
    this._ws.onerror = () => { if (this.onError) this.onError(new Error('kabu WebSocketエラー')); };
  }
  ...
  _handle(d) {
    ...
    if (d.CurrentPrice != null) {
      this._emit({ t: now, type: 'tick', p: d.CurrentPrice, q: d.TradingVolume ? 0 : (d.LastQty || 0), side: d.CurrentPriceChangeStatus === '0058' ? 'S' : 'B' });
    }
    if (d.Sell1 && d.Buy1) {
      const asks = [], bids = [];
      for (let i = 1; i <= 10; i++) {
        const s = d['Sell' + i], b = d['Buy' + i];
        if (s) asks.push([s.Price, s.Qty]);
        if (b) bids.push([b.Price, b.Qty]);
      }
      this._emit({ t: now, type: 'board', asks, bids, over: d.OverSellQty || 0, under: d.UnderBuyQty || 0 });
    }
  }
  _emit(ev) { if (this.onEvent) this.onEvent(ev); }
}
...
class BridgeSource {
  ...
  async start() {
    await new Promise((resolve, reject) => {
      ...
      this._ws.onopen = () => {
        this._ws.send(JSON.stringify({ cmd: 'subscribe', symbol: this.symbol, market: 'JP' }));
        resolve();
      };
      this._ws.onerror = () => reject(new Error('ブリッジWS接続失敗: ' + this.url));
      this._ws.onmessage = (m) => {
        try {
 ...
 * idToken があればAPI取得、なければCSV貼り付けデータを使用。
 * 注意: J-Quants APIはCORS制限がある場合、CSV貼付を利用。
 */
class JQuantsSource {
  constructor(opts = {}) {
    this.symbol = opts.symbol;
  ...
  async start() {
    ...
    if (this.pasted.trim()) {
      ...
      const url = `https://api.jquants.com/v1/prices/daily_quotes?code=${code}&date=${this.date.replace(/-/g, '')}`;
      const r = await fetch(url, { headers: { Authorization: 'Bearer ' + this.token } }).catch(() => null);
      if (!r || !r.ok) throw new Error('J-Quants API取得失敗(CORS制限の場合はCSV貼付を利用してください)');
      const j = await r.json();
      const q = (j.daily_quotes || [])[0];
      if (!q) throw new Error('指定日のデータがありません');
      ohlcv = { o: q.Open, h: q.High, l: q.Low, c: q.Close, v: q.Volume };
    } else {
      throw new Error('idTokenまたはCSVデータを入力してください');
    }
  ...
  _parsePaste(text) {
    ...
    try {
      ...
      return { o: +q.Open || +q.open, h: +q.High || +q.high, l: +q.Low || +q.low, c: +q.Close || +q.close, v: +q.Volume || +q.volume || 1000000 };
    } catch (e) { /* not JSON */ }
    ...
    throw new Error('CSV形式を解析できません(例: 2026-01-12,3000,3080,2980,3050,1234500)');
  }
  ...
  _generate(ohlcv) {
    ...
    const N = 3000; // ティック数
    const day = this.date ? new Date(this.date + 'T09:00:00+09:00').getTime() : Date.now() - 5 * 3600e3;
    ...
    const anchors = [[0, o], [hIdx, h], [lIdx, l], [N - 1, c]].sort((a, b) => a[0] - b[0]);
    ...
    for (let i = 0; i < N && !this._stopped; i++) {
      ...
      const q = Math.max(100, Math.round(qBase * (0.3 + Math.random() * 1.7) / 100) * 100);
      this._emit({ t: times[i], type: 'tick', p, q, side: p >= prev ? 'B' : 'S' });
  ...
  _mkBoard(t, price) {
    ...
    const asks = [], bids = [];
    for (let i = 1; i <= 10; i++) {
      asks.push([price + i * ts, Math.max(100, Math.round((200 + Math.random() * 3000) / 100) * 100)]);
      bids.push([price - i * ts, Math.max(100, Math.round((200 + Math.random() * 3000) / 100) * 100)]);
    }
...
function createSource(kind, opts) {
  switch (kind) {
    case 'simulator': return new SimulatorSource(opts);
    case 'kabu': return new KabuSource(opts);
    case 'bridge': case 'moomoo': return new BridgeSource(opts);
    case 'jquants': return new JQuantsSource(opts);
