...
const App = {
  ...
  playback: {                // practiceモードのリプレイ
    events: [], idx: 0, playing: false, speed: 2,
    timer: null, startT: 0, endT: 0
  },
  chartFrame: 10,
  _renderTimer: null,
  ...
  async boot() {
    ...
    if (ok) this.showHome(); else this.showScreen('screen-login');
  },
  ...
  bindLogin() {
    ...
    $('#btn-login').addEventListener('click', () => doAuth((u, p) => Auth.login(u, p)));
    $('#btn-register').addEventListener('click', () => doAuth((u, p) => Auth.register(u, p)));
  ...
  /* ══════════ ホーム ══════════ */
  bindHome() {
    $$('.home-tab').forEach(btn => btn.addEventListener('click', () => {
    ...
    $('#btn-quick-practice').addEventListener('click', () => this.startQuickPractice());
    $('#btn-quick-support').addEventListener('click', () => this.enterTrade('support', null));
  },
  ...
  async startRecording() {
    ...
    if (kind === 'simulator') sourceOpts.basePrice = parseFloat($('#rec-base-price').value) || 3000;
    ...
    if (kind === 'jquants') {
      ...
      sourceOpts.token = $('#rec-jq-token').value.trim();
    ...
    recorder.onUpdate = (ev) => {
      if (ev.type === 'error') { statusEl.className = 'rec-status'; statusEl.textContent = '⚠ ' + ev.message; return; }
      if (ev.type === 'gen-done') { this.stopRecording(); return; }
      if (ev.type === 'tick') {
        ...
        const dEl = $('#mon-diff');
        dEl.textContent = (diff >= 0 ? '+' : '') + fmtPrice(Math.abs(diff)) + ` (${(diff / first * 100).toFixed(2)}%)`;
        dEl.className = 'mon-diff ' + (diff > 0 ? 'up' : diff < 0 ? 'down' : 'flat');
        const tapeEl = $('#mon-tape');
        const dir = diff >= 0 ? 'up' : 'down';
        tapeEl.insertAdjacentHTML('afterbegin', `<div class="t-${dir}">${fmtTime(ev.t)}  ${fmtPrice(ev.p)}  ${ev.q}</div>`);
        while (tapeEl.children.length > 50) tapeEl.lastChild.remove();
      ...
      $('#mon-elapsed').textContent = fmtDur((Date.now() - recorder.startedAt) / 1000);
    };

    try {
      await recorder.start({ sourceKind: kind, sourceOpts, symbol, symbolName, recDate });
      statusEl.className = 'rec-status recording';
      statusEl.textContent = kind === 'jquants' ? '過去データ生成中…' : '記録中… (' + symbol + ')';
  ...
  async loadLibrary() {
    ...
    try {
      ...
      const mine = all.filter(r => r.user_id === Auth.user.id && !r.deleted && r.status === 'done');
      mine.sort((a, b) => (b.created_at || 0) - (a.created_at || 0));
      if (!mine.length) { el.innerHTML = '<p class="empty">記録がありません。レコーダーで作成するか、クイックスタートで即練習できます。</p>'; return; }
      const srcLabel = { simulator: 'シミュ', kabu: 'kabu', moomoo: 'moomoo', jquants: 'J-Quants' };
      ...
      el.querySelectorAll('.btn-lib-practice').forEach(b =>
        b.addEventListener('click', () => this.enterTrade('practice', b.dataset.rid)));
  ...
  async loadStats() {
    try {
      ...
      const mine = all.filter(s => s.user_id === Auth.user.id && !s.deleted && s.status === 'closed');
      mine.sort((a, b) => (b.created_at || 0) - (a.created_at || 0));
      ...
      $('#stats-sessions').innerHTML = mine.slice(0, 30).map(s => `
        <div class="sess-row">
        ...
        </div>`).join('') || '<p class="empty">セッション履歴なし</p>';
    } catch (e) { /* ignore */ }
  },
  ...
  bindTrade() {
    ...
    $$('.mode-btn').forEach(btn => btn.addEventListener('click', () => {
      ...
      if (m === 'live') { alert('③取引モード(実売買)は今後実装予定です。kabu.com / moomoo APIと連携します。'); return; }
    ...
    $$('.pb-speed').forEach(b => b.addEventListener('click', () => {
      $$('.pb-speed').forEach(x => x.classList.remove('active'));
      b.classList.add('active');
      this.playback.speed = parseFloat(b.dataset.speed);
    }));
    $('#pb-seek').addEventListener('input', () => this.seekTo(parseInt($('#pb-seek').value)));

    // チャート時間軸
    $$('.cframe').forEach(b => b.addEventListener('click', () => {
    ...
    // 注文フォーム
    $$('.otype').forEach(b => b.addEventListener('click', () => {
      $$('.otype').forEach(x => x.classList.remove('active'));
      b.classList.add('active');
    }));
    $$('.qty-btn').forEach(b => b.addEventListener('click', () => {
      const el = $('#ord-qty');
      el.value = Math.max(100, (parseInt(el.value) || 100) + parseInt(b.dataset.dq));
    }));
    $$('input[name=ord-ptype]').forEach(r => r.addEventListener('change', () => {
      $('#ord-price').disabled = $('input[name=ord-ptype]:checked').value !== 'limit';
    }));
    $('#btn-buy').addEventListener('click', () => this.placeOrder('buy'));
    $('#btn-sell').addEventListener('click', () => this.placeOrder('sell'));
    $('#btn-close-all').addEventListener('click', () => { this.engine.closeAll(); this.renderAccount(); });
    ...
    $('#positions').addEventListener('click', (e) => {
      ...
      if (pos) {
        ...
        this.engine._execute(side, pos.type, pos.qty, side === 'buy' ? this.market.bestAsk() : this.market.bestBid());
        this.renderAccount();
      }
    });
    $('#open-orders').addEventListener('click', (e) => {
      const btn = e.target.closest('.btn-cancel-order');
      if (btn) { this.engine.cancelOrder(btn.dataset.oid); this.renderAccount(); }
    });

    // 時計
    setInterval(() => {
      const t = this.market.simTime;
      $('#trade-clock').textContent = t ? fmtTime(t) : new Date().toTimeString().slice(0, 8);
    }, 500);
  },
  ...
  async enterTrade(mode, recordingId) {
    ...
    this.engine.reset();
    this.stopPlayTimer();
    if (this.liveSource) { this.liveSource.stop(); this.liveSource = null; }

    let symbol = $('#rec-symbol').value.trim() || '7203';
    let symbolName = $('#rec-symbol-name').value.trim() || '';

    if (mode === 'practice' && recordingId) {
      const rec = await api.get('recordings', recordingId);
      if (!rec) { alert('記録が見つかりません'); return; }
      symbol = rec.symbol; symbolName = rec.symbol_name;
      $('#playback-bar').style.display = 'flex';
      const events = await loadRecordingEvents(recordingId);
      if (!events.length) { alert('この記録にはデータがありません'); return; }
      ...
      this.liveSource = createSource('simulator', { basePrice: parseFloat($('#rec-base-price').value) || 3000 });
      ...
      const kind = $('#rec-source').value === 'jquants' ? 'simulator' : $('#rec-source').value;
      ...
      this.liveSource.onEvent = (ev) => this.onLiveEvent(ev);
      try { await this.liveSource.start(); }
      catch (e) { alert('データソース接続失敗: ' + e.message + '\nシミュレーターに切り替えます。');
  ...
  switchMode(m) {
    ...
    this.mode = m;
    this.setModeUI(m);
    if (this.engine.session) api.patch('sessions', this.engine.session.id, { mode: m }).catch(() => {});
  },

  setModeUI(m) {
    document.body.dataset.mode = m;
    $$('.mode-btn').forEach(b => b.classList.toggle('active', b.dataset.mode === m));
    const banner = { practice: '練習モード(仮想売買)', support: 'サポートモード(仮想売買+AI分析)', live: '取引モード(実売買)' };
  ...
  exitTrade() {
    ...
    if (this.engine.session) {
      // セッション放棄(記録せず閉じる)
      api.patch('sessions', this.engine.session.id, { status: 'closed', ended_at: new Date().toISOString(), end_cash: this.engine.cash, realized_pnl: this.engine.realizedPnl, trade_count: this.engine.fills.filter(f => f.action === 'close').length, win_count: this.engine.fills.filter(f => f.action === 'close' && f.pnl > 0).length }).catch(() => {});
  ...
  togglePlay(force) {
    ...
    $('#pb-play').innerHTML = pb.playing ? '<i class="fa-solid fa-pause"></i>' : '<i class="fa-solid fa-play"></i>';
  ...
  playLoop() {
    ...
    $('#pb-time').textContent = `${fmtDur((ev.t - pb.startT) / 1000)} / ${fmtDur((pb.endT - pb.startT) / 1000)}`;
    // 次イベントまでの実時間
  ...
  async endSession() {
    if (!confirm('セッションを終了して結果を記録しますか?(未決済建玉は自動決済)')) return;
    this.stopPlayTimer();
    if (this.liveSource) { this.liveSource.stop(); this.liveSource = null; }
    clearInterval(this._renderTimer);
  ...
  renderAll() {
    ...
    Views.renderChart($('#main-chart'), $('#vol-chart'), m, this.chartFrame, this.engine);
    this.renderAccount();
  },
  ...
  renderAccount() {
    ...
    rp.className = e.realizedPnl > 0 ? 'pnl-pos' : e.realizedPnl < 0 ? 'pnl-neg' : '';
