...
class Recorder {
  ...
  async start({ sourceKind, sourceOpts, symbol, symbolName, recDate }) {
    ...
    this.source.onError = (err) => {
      if (this.onUpdate) this.onUpdate({ type: 'error', message: err.message })
    }
    if (this.source instanceof JQuantsSource) {
      this.source.onDone = () => {
        if (this.onUpdate) this.onUpdate({ type: 'gen-done' })
      }
    }
    try {
      ...
      this.active = false
      try {
        await api.remove('recordings', rec.id)
      } catch (_) {
        /* ignore */
      }
      throw e
  ...
  async stop() {
    ...
    if (this._saveErrors.length) {
      ...
      await api
        .patch('recordings', this.recording.id, {
          status: 'error',
          meta: JSON.stringify({ errors: this._saveErrors }),
        })
        .catch(() => {})
      this.recording = null
      this.source = null
    ...
    const rec = await api.patch('recordings', this.recording.id, {
      ...
      interval_ms:
        this.tickCount > 1
          ? Math.round((Date.now() - this.startedAt) / this.tickCount)
          : 0,
    })
...
async function loadRecordingEvents(recordingId) {
  ...
  const mine = chunks.filter(
    (c) => c.recording_id === recordingId && !c.deleted,
  )
  mine.sort((a, b) => a.seq - b.seq)
  let events = []
  for (const c of mine) {
    try {
      events = events.concat(JSON.parse(c.data))
    } catch (e) {
      /* skip broken */
    }
  }
