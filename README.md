...
- `wrangler pages dev` で起動するローカルサーバは、D1の軽量エミュレーターを利用します。
- このローカルD1は本番環境のD1とは別物で、ローカルのファイルとして動作します。
- ローカルで保存したデータは本番D1には反映されません。

### DBの初期化方法（スキーマ適用）

ローカルD1のスキーマを初期化・更新するには以下のコマンドを実行してください。
...
- VS Codeのデバッグ構成で「カブコン: ローカルD1で起動」を選択すると、ローカルD1エミュレーター環境で起動します。
- この状態でコード変更をテストできます。

### 本番データの反映

- 本番D1の最新データをローカルに反映する方法は現状ありません。
- ご自身でデータのエクスポート・インポート機構を別途ご用意ください。
